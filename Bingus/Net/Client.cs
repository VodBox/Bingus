...
namespace Bingus
{
    public class Client : NetoClient
    {
        ...
        private void entireBingoBoardUpdate(ClientModel? _, ServerEntireBingoBoardUpdate boardUpdate)
        {
            if (Room != null)
            {
                Room.Match.Board = boardUpdate.SizeX > 0 && boardUpdate.SizeY > 0 && boardUpdate.Squares.Length == boardUpdate.SizeX * boardUpdate.SizeY ?
                    new BingoBoard(boardUpdate.SizeX, boardUpdate.SizeY, boardUpdate.Lockout, boardUpdate.Squares, boardUpdate.AvailableClasses) :
                    null;
            }
        }
